...
class PaymentController extends BasePaymentModuleController
{
    ...
    public function processManualResponse(): void
    {
        $this->getLog()->addInfo(
            $this->getTranslator()->trans(
                "AtosV2 manual response processing.",
                [],
                AtosV2::MODULE_DOMAIN
            )
        );

        $paymentResponse = new AtosV2Api(AtosV2::getConfigValue('secretKey'));
        ...
        $order = OrderQuery::create()
            ...
            ->filterByPaymentModuleId(AtosV2::getModuleId())
            ->findOne();
     ...
     * @throws \Exception
     */
    public function processAtosV2Request()
    {
        $this->getLog()->addInfo(
            $this->getTranslator()->trans(
                "AtosV2 automatic response processing.",
                [],
                AtosV2::MODULE_DOMAIN
            )
        );
        ...
        if ($paymentResponse->isValid()) {
            if (null !== $order = OrderQuery::create()
                ...
                if ($paymentResponse->isSuccessful()) {
                    ...
                    $this->getLog()->addError(
                        $this->getTranslator()->trans('Order was canceled.', [], AtosV2::MODULE_DOMAIN)
                    );
                }
            } else {
                $this->getLog()->addError(
            ...
            $this->getTranslator()->trans(
                'Got invalid response from AtosV2',
                [ ],
                AtosV2::MODULE_DOMAIN
            );
        }
    ...
    public function processUserCancel($orderId): void
    {
        ...
        $this->redirectToFailurePage(
            $orderId,
            $this->getTranslator()->trans('you cancel the payment', [], AtosV2::MODULE_DOMAIN)
        );
    }
